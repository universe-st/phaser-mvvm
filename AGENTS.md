...
- **滚动与焦点**：`#/scroll`（`apps/examples/src/scenes/scroll.ts`）是 `ScrollView` 的常驻验收场：220 行虚拟化列表 + 横向 chip 条（**30 个可聚焦 `Button`**，`focusOrder: -1`）+ 一个开了 pinch zoom 的嵌套口。`window.scrollDemo` 提供 `offsets()`/`visibleRowKey()`/`owners()`/`zoom()`/`setZoom()`/`viewports()`；`#demo-state` 逐帧发布 `v.*`/`h.*`/`nested.*`/`chip` 与**焦点几何**（`focus.x/y/w/h`）以及三个口的可见带（`v.top`/`v.height`、`h.left`/`h.width`、`nested.*`）。**核心判据**：① 拖动归属空闲时必须是 `none`（V24）；② **整个 `Tab`/`Shift+Tab`/`D-Pad` 走查里焦点从不落在可见带外面**，且每步偏移只前进一行（V33）；③ 口滚到尽头后方向键要能**离开**口（V35）；④ 在 chip 条上拖动 300px 不激活任何 chip（`chip=none`）。矩阵见 [`ACCEPTANCE-scroll.md`](./docs/ACCEPTANCE-scroll.md)。
