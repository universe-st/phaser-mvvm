...
- **几何 + 像素验收**：`node scripts/visual-check.mjs`。脚本用**单个**无头 Chrome（CDP + 固定视口 1280×720）逐场景：读页面 `#status`（场景把引擎实际分配的 rect 写进 DOM）→ 截图 → 按 canvas 偏移换算后采样中心像素并与期望填充色比对。常用参数：`--no-build`、`--size 1024x768`、`--port`、`--out .tmp/visual-check`；需要 `CHROME_PATH` 或本机 Chrome/Chromium，**仅 macOS/Linux 可用**。
  - 要校验的场景在脚本里**硬编码**（当前 `m0`、`probe`、`stack`、`hud`、`modal`、`uiscene`）——新增场景若需像素断言，必须同时加进 `PIXEL_EXPECTATIONS` 与该场景列表。
...
- **插件选项**：`#/config`（`apps/examples/src/scenes/config.ts`）是 `MVVMPlugin.configure`（游戏级默认值，创建游戏前）与 `mvvm.configure`（运行期补丁）的常驻验收场。`window.config` 提供 `patch(options)`/`defaults()`/`cameraColor()`/`liveAttrs()`/`theme(name)`/`focus(name)`/`focusName()`/`router()`/`point(name)`/`state()`。**关键判据**：`MVVMPlugin.configure` 设的默认值真的到达场景（`aria-live="assertive"`）、`configure({ navigation: false })` 立刻停掉 Tab 但鼠标照常、子选项包一层深合并（改 `focus` 不会清掉 `input`）、`themeBackground: false` 后切主题相机颜色不再跟随。**安全区**：`window.config.safeArea()` 与 `#demo-state` 的 `safeArea.*`/`root.padding.*` 给出"浏览器报的 inset"与"根留出的内边距"，`window.config.resize()` 强制重读（inset 只在 resize 时重读）——这一条只能逐设备验，矩阵见 [`ACCEPTANCE-mobile.md`](./docs/ACCEPTANCE-mobile.md)。矩阵见 [`ACCEPTANCE-config.md`](./docs/ACCEPTANCE-config.md)。
