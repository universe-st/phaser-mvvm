...
- **指针落点走树有 Node 单测**：`packages/phaser/test/target-walk.test.ts` 覆盖 `resolveTargetInTree()`（最深优先、隐藏子树跳过、容器链偏移、每个候选自己的坐标空间等 10 条）——这是 V1/V8/V9 三次缺陷的所在，改动 `InputRouter` 的命中逻辑（含 `pointerInWidgetSpace`）时先跑它，再用浏览器验收。
