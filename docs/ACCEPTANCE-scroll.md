...
- **真实触摸设备**：本轮的手势全部在 CDP 触摸仿真与鼠标上跑过（§2.7 的 S18 是触摸拖拽链式传递），**Android 模拟器上已验收真实触摸拖动**（第 112 轮 A3：`input swipe` → `offset 0 → 438`，挂载行 14 → 17 / 共 220 行，见 [`ACCEPTANCE-android.md`](./ACCEPTANCE-android.md)）；物理设备未跑。
