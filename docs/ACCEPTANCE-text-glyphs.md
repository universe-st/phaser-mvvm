...
## 4.5 同一条线上的第二个缺陷：中文长句不换行（第 56 轮）

给本章加「换行/截断边界」探针时量出来的：`Text(中文长句, { width: 140, wrap: true })` 的盒子**只有 19.5px 高**——一行。Phaser 的换行只在空格处断行，中文没有空格，于是整段横向溢出容器约 4 倍宽。修法是在 Phaser 之后补一道**逐码点兜底断行**（`rewrapOverflowingLines`，即 CSS 的 `overflow-wrap: anywhere`），先于 `maxLines`/省略号执行；单个字形宽过盒子时独占一行。

| 探针（盒子宽 140）                   | 修复前                                | 修复后                                          |
| ------------------------------------ | ------------------------------------- | ----------------------------------------------- |
| 36 汉字长句                          | 盒高 **19.5**、横向溢出               | 盒高 **89.5**（≈5 行）、墨迹右缘 128 < 140      |
| 40 字符无空格 ASCII                  | 盒高 19.5、溢出                       | 盒高 **54.5**（3 行）、墨迹右缘 136             |
| 同上 + `maxLines: 1, ellipsis: true` | 盒高 19.5（省略号生效但仍是整串宽度） | 盒高 **19.5**、墨迹 133（一行、省略号收在盒内） |

探针留在 `#/compose` 的 Text 分区并在 `#status` 里报出盒子矩形（`textwrap.cjk` / `textwrap.word` / `textwrap.ellipsis`），因此这条回归以后可以随时重跑。
