...
| `focus.wrap`/`trapFocus`/`ring`                            | 直接写活动焦点管理器；**`ring` 第 113 轮起真的生效**（此前只写不读、是个死选项 = V79）：它**立刻重画当前持有焦点的那个控件**，只抑制焦点环、不动焦点与光标（`#/states` 实测 `{focused:true,ringOn:false}`，见 [ADR-0012](./adr/0012-focus-visible-ring.md)；常驻像素门禁是 `visual-check` 的 `states.ringGate` 臂——键盘聚焦滑杆后切这个开关，环必须**真的从画面上消失**，见 [`ACCEPTANCE-states.md`](./ACCEPTANCE-states.md) §10.1） |
