...
- ~~**没有 `NavSource` 这个符号**~~ **现状校正（第 110 轮）**：这条写于第 66 轮前后，当时交付的只有 `nav.ts` 的纯函数（`NavRepeat`、`keyboardActionOf`、`gamepadActionsOf`、`heldDirectionsOf`、`GAMEPAD_BUTTON_ACTIVATE/BACK`）由 `FocusManager` 消费。第 110 轮把「具名抽象」补上了：`NavSource`/`NavSourceHost`/`NavSourceRegistry`（`packages/phaser/src/nav.ts`）+ 内置 `KeyboardNavSource`/`GamepadNavSource`（`nav-sources.ts`），插件侧 `registerNavSource()`/`unregisterNavSource()`/`navSources`，`#/gallery` 常驻验收（[`ACCEPTANCE-gallery.md`](../ACCEPTANCE-gallery.md) §6）。原决策（键盘/手柄/D-Pad 的语义由 `FocusManager` 消费）未变，只是那层「谁来提供设备」现在有了名字。
