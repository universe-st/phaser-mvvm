...
| 23 个场景    | 冷启动（含 `m0`/`probe`/`stack`/`dashboard`/`compose`/`options`/`config`/`a11y`/`lifecycle`：本轮把 `events` 也加进了注册表，所以扫描的是全部 23 个） | 画布就绪、`#status` 与 `#demo-state` 有内容、**console 无 error / pageerror**                                        |
