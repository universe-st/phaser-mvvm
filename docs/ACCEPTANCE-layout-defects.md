...
> 结论：**L1–L8 全部修复并带回归测试；W2 先在真实浏览器里复现（偏移 224.5 / limit 0）再修复并复测通过；W1 增加祖先回退与明确告警；同一轮内追加修复疑似项 V5、V6 与文档项 W3。布局黄金快照零漂移，全仓 963 个用例通过。**
...
## 3.3 追加：疑似项 V5 / V6 / W3（同一轮内确认并修复）

| #   | 缺陷                                                                                                                                                                  | 修法                                                                        | 回归测试                                                                         |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| V5  | 转换器字符串字面量**二次反转义**（先解 `\\` 再解 `\n`）：`{{ p \| default('C:\\new') }}` 渲染成 `C:` + 换行 + `ew`                                                    | `template.parseLiteral` 改为单趟解转义（查表）                              | `template literals · escaping`（2 例：字面反斜杠保留、`\n`/`\t`/转义引号仍生效） |
| V6  | `registerConverter` 会 trim 名字，而 `hasConverter`/`unregisterConverter`/`applyConverter` 不会 → `registerConverter(' money ', fn)` 注册出来的转换器查不到、也用不了 | 四个入口统一 `converterKey()`（trim）；未知转换器的报错也打印 trim 后的名字 | `converter registry · name handling`（2 例）                                     |
| W3  | `ThumbGeometry.position` 文档写「轨道比例 0…1」，实现与消费方都是**像素**                                                                                             | 修正 JSDoc                                                                  | 无需（纯文档）                                                                   |
