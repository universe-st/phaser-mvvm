...
## 5. 点击穿透到游戏世界（V9，第 43 轮已修复）

第 38 轮在这里写道：UI 上的点击**会**穿透到 UI 之下的 Phaser 游戏对象（点顶栏空白、底栏空白、世界区域、`加分` 按钮，世界背景矩形都收到 `pointerdown`），PLAN §9 把「Panel 默认带拦截层」列为对策，而当时它只管控件之间。第 43 轮修好了，矩阵如下（`#/hud`，页面的 `blockPointer: false`，顶栏/底栏保留默认的拦截层）：

| 点击位置                  | `world.clicks` 增量 | `clicks` 增量 | 结论                                    |
| ------------------------- | ------------------- | ------------- | --------------------------------------- |
| 顶栏面板体（600,28）      | **0**               | 0             | 拦截层吞掉，不再穿透                    |
| 底栏面板体（700,615）     | **0**               | 0             | 同上                                    |
| 世界区域（700,380）       | **+1**              | 0             | 该层声明 `blockPointer:false`，正常穿透 |
| `加分` 按钮（`pt.score`） | **0**               | +1            | UI 消费这次按下                         |
| 触摸点击同一按钮          | **0**               | +1            | 触摸路径同样不穿透                      |

机制：`InputRouter` 不再要求「目标控件自己收到事件」，而是从坐标解析目标（事件只当作"UI 内发生了一次按下"的信号），于是 Phaser 的 `topOnly` 可以保持开启——只有最上层的交互对象（UI）收到事件，UI 之下的游戏对象不再收到。第 39 轮否决的两条捷径（裸开 `topOnly`、事件容器 `stopPropagation()`）与完整对比见 [`DEFECT-BACKLOG.md`](./DEFECT-BACKLOG.md) V9。

**行为变化（面向使用者）**：落在任何 UI 目标上的按下不再传给游戏世界；需要穿透的层（例如全屏透明 HUD 页）写 `blockPointer: false`。`Panel` 的默认值没有变。

## 5.5 自动像素门禁（第 52 轮）

上面的矩阵要人（或 Playwright MCP）驱动；第 52 轮把这一页加进了 `scripts/visual-check.mjs`（无头 Chrome + CDP，可在 CI 跑），让「相机钉住的 HUD」有了一条**不依赖交互**的回归线：

| 检查           | 断言                                                              | 为什么它有意义                                                               |
| -------------- | ----------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| `canvas.clear` | 画布内 (340,60)（两块瓦片之间的 2px 缝，画布相对坐标）= `#0d1117` | 相机背景色，且该点确实在世界里                                               |
| `score`        | 主按钮被采样点（`fx: 0.12`，避开文案）= `#2f6feb`                 | **钉住的 HUD 仍画在布局坐标上**——相机滚到 (260,140) 也不动                   |
| `tile`         | 页面 (700,380) = `#161b22`                                        | 该点是世界瓦片 (4,2)，其颜色由瓦片网格决定：**只有相机真的滚动了才是这个值** |

场景在截图前通过脚本的 `SCENE_SETUP` 把相机滚到 (260,140)。**灵敏度验证**（不是空断言）：把 setup 改回 `scroll(0,0)` 再跑，`tile` 立刻变成 `MISMATCH expected #161b22 got #1b2230`（滚动前该处是另一块瓦片），`canvas.clear` 同时因为缝的位置变了而失败；恢复后全绿。
