...
| 检查     | 结果                                                                                                                                                                                                                           | 怎么复现                                                                                                                       |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ |
| 单测     | **1409 passed**：core 281、layout 314、phaser 418、widgets 396（第 113 轮 +11：`test/focus-visible.test.ts` 的焦点可见性用例；第 110 轮那 +16 见下表）                                                                         | `pnpm -r run test`                                                                                                             |
...
| 场景     | 注册表 **23 个场景**；`visual-check` 硬编码 **15 个场景名**（13 个页面，`#/states` 以别名出现两次做焦点环 A/B）：**108 个像素检查（54×2）+ 4 张 AX 表 + 2 条输入断言全部 OK**                                                  | `node scripts/visual-check.mjs`（受限沙箱里 Chrome 可能起不来，见 AGENTS §6）                                                  |
...
| #   | 项目                                    | 结果                                                                                                                                                                                                                                                                                        | 证据                                                                                                                      |
| --- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| 1   | **V81：焦点"可见性"从未被建模**         | `Widget#focusVisible` 成为**画环的唯一判据**（四个画环点全部改判）；指针按下只移动焦点不画环（CSS `:focus-visible`），键盘/手柄/程序化焦点照旧画；文本域用 `Widget#focusRingOnPointer` opt-in 保留环；`focused`/`visualState` 语义**一字未改**                                              | [ADR-0012](./adr/0012-focus-visible-ring.md)、[`PITFALLS.md`](./PITFALLS.md) §8.74、`test/focus-visible.test.ts`（11 条） |
...
| 4   | **焦点环的像素门禁（同轮补上）**        | `visual-check` 把 `#/states` 跑**两遍**（`states.pointer` / `states.tab`：同一个 hash、同一个按钮、同一次主题切换）：真实指针点击后按钮顶边必须与没被碰过的邻居**同色**（= 没画环），真实 `Tab` 走到它之后必须读到环色；两条各带一条输入断言，明暗两套共 **+12 个像素检查**，场景名 13 → 15 | `node scripts/visual-check.mjs`，阳性对照与实测数字见 [`ACCEPTANCE-states.md`](./ACCEPTANCE-states.md) §10.1              |
