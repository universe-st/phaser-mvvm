...
## 1.1 当前状态（第 110–113 轮：M9 收尾 + 公开 API 冻结 + 范围收紧到 Android + Android 模拟器验收 + 焦点可见性）

| 检查     | 结果                                                                                                                                                                                                                           | 怎么复现                                                                                                                       |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ |
| 单测     | **1417 passed**：core 281、layout 314、phaser 426、widgets 396（第 113 轮 +19：`test/focus-visible.test.ts` 的焦点可见性用例；第 110 轮那 +16 见下表）                                                                         | `pnpm -r run test`                                                                                                             |
...
| API 冻结 | **817 个导出名**（5 个入口点）与 `docs/API-SURFACE.json` 一致；四个包版本 `1.0.0`。第 113 轮的焦点可见性**没有新增导出名**，所以快照未动                                                                                       | `pnpm api:check`（ADR-0011）                                                                                                   |
| 体积     | 均在预算内：core+layout **18.6 KB**、phaser+widgets **33.1 KB**（min+gzip，第 113 轮复测；+0.2 KB 来自焦点可见性）                                                                                                             | `pnpm size`                                                                                                                    |
| 场景     | 注册表 **23 个场景**；`visual-check` 硬编码 **17 个场景名**（13 个页面，`#/states` 以**四个**别名出现，做焦点环的四种输入序列）：**116 个像素检查（58×2）+ 4 张 AX 表 + 4 条输入断言全部 OK**                                  | `node scripts/visual-check.mjs`（受限沙箱里 Chrome 可能起不来，见 AGENTS §6）                                                  |
...
### 第 113 轮做了什么（焦点可见性：点过的按钮不再带框）

| #   | 项目                                    | 结果                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 证据                                                                                                                                                                              |
| --- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **V81 + V82：焦点"可见性"从未被建模**   | `Widget#focusVisible` 成为**画环的唯一判据**（四个画环点全部改判）；指针按下只移动焦点不画环（CSS `:focus-visible`），键盘/手柄/程序化焦点照旧画；文本域用 `Widget#focusRingOnPointer` opt-in 保留环；`focused`/`visualState` 语义**一字未改**。**V82 是集成到消费方（揭棋）时用户当场看出来的**：① 按下**已经持有焦点**的控件不降级（对话框 `focusFirst` 给出的环，用户点它不消失）；② `Slider` 的重绘缓存键漏了 `focusVisible`（切 `ring` 开关不重画）；③ **V83：没有来源的焦点变化跟随最后一次输入的模态**——触摸游戏点开音量对话框时，第一个滑杆不再戴框（那一下框是"上一次点击的后果"），键盘/手柄走到的控件照旧有框 | [ADR-0012](./adr/0012-focus-visible-ring.md)、[`PITFALLS.md`](./PITFALLS.md) §8.74、`test/focus-visible.test.ts`（13 条）、[`ACCEPTANCE-states.md`](./ACCEPTANCE-states.md) §10.1 |
| 2   | **V79 的另一半：`focus.ring` 是死选项** | `FocusManager#ring` 接通成真开关（此前被写入字段、从无一处读取），运行期写入立刻重画当前聚焦控件；只抑制环、不动焦点与光标                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | [`ACCEPTANCE-config.md`](./ACCEPTANCE-config.md) §1、[`ACCEPTANCE-states.md`](./ACCEPTANCE-states.md) §10                                                                         |
| 3   | **`#/states` 新增焦点可见性读数**       | `#demo-state` 逐帧发 `ring.<name>=on\|off`；`window.states.ring()` 读实时的 `{ focused, ringOn }`（不受"只在变化时写"影响，检查无需等帧）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 真鼠标 / 真 `Tab` / 真点击文本框 / 运行期切 `ring` 五条 A/B，见 [`ACCEPTANCE-states.md`](./ACCEPTANCE-states.md) §10                                                              |
| 4   | **焦点环的像素门禁（同轮补上）**        | `visual-check` 把 `#/states` 跑**四遍**（`states.pointer` / `states.tab` / `states.pressFocused` / `states.ringGate`：同一个 hash、同一次主题切换，只差输入序列）：点过的必须与邻居**同色**、`Tab` 到的必须读环色、**先键盘聚焦再点它**与**先键盘聚焦再切 `ring: false`** 都必须让环消失；四条各带一条输入断言，明暗两套共 **+20 个像素检查**，场景名 13 → 17。三个阳性对照分别打红不同的臂                                                                                                                                                                                                                              | `node scripts/visual-check.mjs`，阳性对照与实测数字见 [`ACCEPTANCE-states.md`](./ACCEPTANCE-states.md) §10.1                                                                      |
