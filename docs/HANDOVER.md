...
**下一个会话请直接从第 4 节的第 1 项（V1）开始**，必要时先做一次第 1 节的验证跑一遍确认基线；`docs/DEFECT-BACKLOG.md` 有每一项的原始描述与已排除的路径。
