...
- **真机验收范围限定 Android**（第 111 轮的范围收紧）：真机只验 **Android Chrome**（触摸、DPR、安全区、软键盘、帧率）；**iOS Safari / iPadOS、桌面 OS 真机、真实屏幕阅读器（VoiceOver / NVDA / TalkBack）、真实手柄硬件一律不在验收范围内** —— 这是有意排除，不是待办。替代证据分别是 Chrome 设备仿真 + CDP 触摸域、**CDP 算出的可访问性树**（`Accessibility.getFullAXTree` → `visual-check` 的 `AX_EXPECTATIONS` / `AX_STRUCTURE_EXPECTATIONS`）、以及假手柄夹具（`apps/examples/src/fake-pad.ts` 的 `window.fakePad`）与 `packages/phaser/test/nav.test.ts`。各验收记录里标题为「未验证 / 已知边界」的小节按这个口径读。**Android 这一条已在模拟器上验收完成**（第 112 轮：android-cli + Cordova，`node scripts/android-check.mjs verify` 9/9，见 [`ACCEPTANCE-android.md`](./ACCEPTANCE-android.md)）；**物理设备仍未跑** —— 模拟器覆盖不到 GPU/驱动、厂商 IME、挖孔/手势条与绝对帧率。
...
- **像素对齐**：内部保留亚像素位置；在 `arrange` 末尾对**整个矩形**做取整（`LayoutEngineOptions.snapMode: 'none'|'round'|'floor'|'ceil'` 默认 `'round'` + `dpr`；`UIRoot` 把同样的选项暴露给页面）。`dpr` 的含义是**绘制缓冲像素 / 布局单位**，`UIRoot` 的默认值是 `1`——**不是** `window.devicePixelRatio`：Phaser 把画布后备缓冲固定成 `gameSize`，吸附到缓冲表示不了的更细网格只会把每条边放到半像素上、被光栅化成两个半亮像素（V79，见 [`PITFALLS.md`](./PITFALLS.md) §8.72）；把内容渲染得比布局空间更大（相机缩放、容器缩放）时才显式填那个倍率。字形另按**物理像素 / 布局单位**烘焙（`MVVMPlugin#textResolution`）。想让**矢量**绘制也落在设备像素上，则用 `UIRootOptions.designResolution`（`gameSize = 设计 × dpr` + 相机缩放并对准设计框）：根按设计尺寸布局、吸附网格跟随放大倍率、字形烘焙与 DOM 桥都按布局宽换算，见 [`PITFALLS.md`](./PITFALLS.md) §8.73。
...
| **M9 导航与无障碍**（**手柄 + 无障碍镜像 + 手柄文本输入 + `NavSource` 具名抽象已交付**；真机阅读器与真手柄**已移出验收范围**，见 §1.2） | 键盘/手柄导航（`nav.ts` 的按键映射 + 连发，由 `FocusManager` 消费）、`A11yBridge` DOM 镜像 + `aria-live` 播报、`VirtualKeyboard` 屏幕键盘 | `packages/phaser/src/{input,a11y,nav}.ts`、`packages/widgets/src/{VirtualKeyboard,keyboard-plan}.ts` + `#/a11y`/`#/gallery`/`#/keyboard` 示例 | ✅ 手柄能导航/激活/返回，且能操作滑杆与滚动容器（[`ACCEPTANCE-gamepad.md`](./ACCEPTANCE-gamepad.md)）；✅ 每个可交互控件有隐藏 DOM 镜像（role/label/state，镜像与控件树同构）、焦点与消息经 `aria-live` 播报（[`ACCEPTANCE-a11y.md`](./ACCEPTANCE-a11y.md)）；✅ 纯手柄完成**文本输入**（D-Pad 走查 + `A` 打字/退格/提交，鼠标与触摸同样验收，[`ACCEPTANCE-keyboard.md`](./ACCEPTANCE-keyboard.md)）；✅ **`NavSource` 具名抽象**（第 110 轮：可注册第三台设备，`#/gallery` 常驻验收，[`ACCEPTANCE-gallery.md`](./ACCEPTANCE-gallery.md) §6）；~~⬜ 真实屏幕阅读器听过一遍；⬜ 真实手柄硬件~~ **这两项第 111 轮已移出验收范围**（见 §1.2），M9 的验收标准因此全部满足 | 3–5 天 |
...
- 体积（gzip，不含 Phaser）：`core` + `layout` < 25 KB；`phaser` + `widgets` < 45 KB——由 `pnpm size`（`scripts/size-check.mjs`）在 **minify 后**的 gzip 上判定（库产物故意不 minify，便于堆栈可读；消费方打包时一定会 minify），实测 **18.6 KB** / **32.9 KB**（第 112 轮复测；第 107 轮是 18.6 / 31.3 KB，第 111 轮的 `render-scale.ts` 单独量只有 0.5 KB min+gzip，其余差额来自第 107 轮之后的既有增长，含第 112 轮的 `designResolution`），脚本同时打印未压缩 gzip 值（28.1 KB / 67.0 KB）以便对照。第 5 轮首发时是 18.3 / 14.7 KB：`phaser` + `widgets` 的上涨来自第 5 轮之后 phaser 包长大（页面栈/路由/模态/动效/无障碍镜像/虚拟键盘…陆续进来），不是某一个模块的增量——第 107 轮的指针事件链单独量只有 1.6 KB min+gzip。
