...
| ~~`NavSource` 抽象（方向键 / Tab / 手柄 D-Pad+摇杆合成一个具名抽象）~~  | **已交付（第 110 轮）**：`NavSource`/`NavSourceHost`/`NavSourceRegistry` + 内置 `KeyboardNavSource`/`GamepadNavSource` 都在 `packages/phaser/src/{nav,nav-sources}.ts`，插件侧是 `registerNavSource()`/`unregisterNavSource()`/`navSources`（见 [07 §4.1](./07-input-focus-nav.md) 与 [`ACCEPTANCE-gallery.md`](../ACCEPTANCE-gallery.md) §6）。本节此前的说法是「没有这个符号」，那是第 110 轮之前的事实 |
| Android（模拟器 + Cordova）                                             | **已验收（第 112 轮）**：`node scripts/android-check.mjs verify` **9/9** —— 真实触摸、真实系统 IME、无障碍树、泄漏门禁、设备帧缓冲像素，见 [`ACCEPTANCE-android.md`](../ACCEPTANCE-android.md)。**物理设备仍未跑**                                                                                                                                                                                        |
| ~~真实屏幕阅读器（VoiceOver/NVDA/TalkBack）人工走查、真实手柄硬件验证~~ | **已移出验收范围（第 111 轮，有意排除，不是待办）**：见 [`PLAN.md`](../PLAN.md) §1.2 —— 真机只验 **Android Chrome**，真实 SR 与真手柄都不做。替代证据是假手柄驱动（`window.fakePad`）+ **CDP 算出的可访问性树**（`#/gallery`/`#/keyboard`/`#/a11y`，见 [`ACCEPTANCE-a11y.md`](../ACCEPTANCE-a11y.md)）                                                                                                    |
