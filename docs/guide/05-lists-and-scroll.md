...
| 键盘/手柄  | 需要**控件持有焦点**：方向键或手柄 `D-Pad`/左摇杆沿**自己的轴**滚动（纵向控制器只认 `↑↓`，横向只认 `←→`，`direction: 'both'` 四个都认，因此交叉轴仍然用来切换焦点）；`Home`/`End`/`PageUp`/`PageDown` 是键盘专属；`Tab`/`Enter`/`Escape` 从不消费 |
| 滚到尽头   | **拒绝**这个方向：偏移没变就不消费按键，动作回到焦点管理器，焦点因此可以离开这个口。手柄没有 `Tab` 键，不这样做用户就被永久困在口里（[V35](../DEFECT-BACKLOG.md)）                                                                                |
| 焦点进视野 | **焦点走到哪，口就滚到哪**：焦点移到一个被裁掉的行上时，口自动把它滚进可见带并留出边距（默认 8px，见下节）                                                                                                                                        |
| 裁剪       | 视口以外（被遮罩裁掉）的内容**不可点、不可悬停**；在视口里点一下也不会让视口失去拖动能力                                                                                                                                                          |
| 触摸       | 走同一条指针路径（Phaser 的 pointer 抽象）                                                                                                                                                                                                        |

### 4.1 焦点进视野（bring-into-view）

键盘和手柄的焦点是**纯几何**地移动的，它不知道有个遮罩。没有这一步，`Tab` 就会把焦点环画到裁剪区外面：屏幕上什么都看不到，而行还在（`overscan` 让挂载窗口比视口多几行），`Enter` 照按不误——连"点不动"这个线索都没有（第 67 轮的 [V33](../DEFECT-BACKLOG.md)，实测见[验收记录](../ACCEPTANCE-scroll.md)）。

规则抄的是浏览器的 `element.scrollIntoView({ block: 'nearest' })`：

- 目标**装得下**：只对齐被破坏的那条边，滚最短距离；
- 目标**比视口还高**：对齐起始边（对齐尾部会把用户正在读的开头藏起来）；
- 目标**已经盖住整个可见带**：不动（移动内容什么也揭示不了，看着只像随机跳动）；
- `revealMargin` 会被夹到一半余量，否则两条边会互相打架。

```ts
Scroll({ height: 240, revealMargin: 24 }, () => {
  /* … */
}); // 留更宽的呼吸空间
```

滚动**只在焦点变化时**发生：用户自己拖/滚轮翻页时框架不插手。

**自己实现裁剪容器时**照抄 `ScrollView` 的三步：覆盖 `Widget#revealDescendant(target)` → 用 `contentRectOf(target, <内容根>)` 拿**内容坐标**（与当前偏移无关，这正是它能在同一次调用里既读又写偏移的原因）→ 用 `revealOffset()` 算新偏移并 `setOffset()`（它会夹到自己的上下限，并转告虚拟化列表挂载哪一段）。**移动了返回 `true`，被自己的上下限夹回、其实没动就返回 `false`**——后者决定焦点系统要不要为一次没发生的滚动再跑一遍布局。

嵌套口由框架收敛：`revealInViewports()` 从内到外问每一层，一遍有移动就先重跑一次布局再走一遍（最多 3 遍），所以"列表里的卡片滚进内层口、内层口再滚进外层口"能在一次调用里算完。

**嵌套滚动的规则**（`scroll.ts` 与 `list.ts` 里的真实场景）：

- **最内层的可滚动视口赢得手势**：指针下面是内层列表时，外层页面不会同时跟着动（拖拽和滚轮都遵循这条）。
- **用完就向上链式传递**：内层已经到头、或者根本不在该轴上滚（比如纵向页面里的横向条带），**剩下的增量**交给外层 —— 列表滚到底后继续滚/继续拖会接着滚页面，而不是「卡住」。滚轮与拖拽走的是同一条规则（第 70 轮统一：此前只有滚轮会链式传递，于是**触摸设备**上"内层小列表嵌在页面里"会把手势吞掉——手指一直在动，页面一动不动；实测见 [`ACCEPTANCE-scroll.md`](../ACCEPTANCE-scroll.md) §2.7）。
  - 反向也成立：往下拖时只要内层还有余量，就是**内层先动**，外层不动。
  - 手势的归属（`dragPointerId`）仍然只属于最内层那个口，所以抬手、甩动惯性、双指缩放都不受影响。
