...
- **状态**：M0–M8 已交付（响应式内核、布局引擎、适配层、控件库、文本框与表单、绑定与列表、滚动与裁剪、模态/页面栈/`Router`/`UIScene`），M9 已交付手柄导航、无障碍镜像与手柄文本输入。**未做的**：真实屏幕阅读器与真实手柄硬件的人工验证、`NavSource` 这个具名抽象（导航实际在 `phaser/src/nav.ts`）、`@phaser-mvvm/template`（Phase 2）。名字与 PLAN 草案的差异清单见 [08-lifecycle-and-pitfalls.md](./08-lifecycle-and-pitfalls.md) §5。
...
| [02 布局](./02-layout.md)                                   | 卡片页 → 仪表盘 → 网格画廊 → 角标/对话框          | 两阶段 `measure/arrange`、五种容器、`LayoutParams`、脏标记与测量缓存                             |
