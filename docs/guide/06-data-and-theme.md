...
| 方向        | 机制                                        | 为什么安全                                                                                                                                     |
| ----------- | ------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| 模型 → 视图 | 调控件的 `setValue`（**会** emit `change`） | `change` 是"值变了就报"（第 81 轮的 V49：静默写值会让 `ref` 与控件永久不一致）；回环由绑定侧的**等值短路**断掉（写回前比较当前值，相等就跳过） |
| 视图 → 模型 | 监听控件的 `change` 事件                    | 绑定只把**真的变了**的值写回模型；只想要"用户编辑"这一路，用 `onChange` 选项（程序化写入不触发它）                                             |
