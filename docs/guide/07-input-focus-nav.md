...
| 事实           | 说明                                                                                                                                   |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| 谁有节点       | **可交互控件**（`InputRouter` 的注册集合）：按钮、开关、禁用按钮、滑杆、输入框、滚动区、可点击卡片                                     |
| 谁没有节点     | `Text` 这类非控件、装饰性面板（它们没有描述符，由拥有它们的控件读出）                                                                  |
| 节点可聚焦吗   | **不可**（没有 `tabindex`）：键盘/手柄控制权留在框架里，屏幕阅读器在浏览模式里读镜像                                                   |
| 可访问名从哪来 | 控件自己的文字（按钮文案、`placeholder`）优先；**没有文字的控件用 `label` 选项**（`Slider({ label: '音量' })`），否则会读出调试 `name` |
| 什么时候更新   | 结构变化（`refreshInteraction()`，插件自动做）、焦点变化（顺手同步该控件）、以及你显式调 `sync(widget)`（值变了之后）                  |
| 播报什么       | 焦点变化时读"可访问名, 状态/值"；`announce(text)` 读你给的句子（同一条消息重复播报也会被读出）                                         |
