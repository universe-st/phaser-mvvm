...
> 这两条只作为**一次性测量**记录：`#/form` 没有进 `PIXEL_EXPECTATIONS`（脚本虽然会给有亮色期望的场景拍第二张图，但这一页没有采样点），而光标是闪的——放进门禁会变成 flaky。**未进门禁 = 未持续验证**，这一条写在这里而不是假装它被持续守着。
...
### 5.7 拖到边缘自动滚动（V62，第 97 轮）

第 93 轮把拖选做完时留下一条：**把指针拖到字段之外不会滚动任何东西**。浏览器里的原生输入框在按住不动、指针停在框外时会**继续**滚动内容（速度随离框的距离增加），选区因此一小步一小步地长；Canvas 路径只把光标钳到文本末尾，指针一停就不再变化。实测（修前，`rows: 3` 的多行字段、9 行内容、按在框下缘外 20px 并**保持不动**）：`top 0`、`caret 14`、`selection 11` —— 五帧之后还是这三个数。

修法是给那次拖动加一个**逐帧**的推进（`POST_UPDATE`，与 `ScrollView` 的滑行同一个钩子）：指针越过内容框的边就按 `dragAutoScrollStep(overflow, frameDelta)` 滚一步（离框越远越快，刚出框就有 1/6 全速 150 px/s，最快 900 px/s，单帧积分上限 50 ms），光标则取**钳进框内**的指针位置——指针说"哪条边"，滚动说"哪个字符"。`updateScroll()` 在这次拖动期间让位，否则"把光标滚进视野"会一步把内容弹到文本末尾，逐帧推进就退化成了原来的那一次跳转。

| #   | 操作（真鼠标）                                                    | 期望                                            | 实测                                                                                                                          |
| --- | ----------------------------------------------------------------- | ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 1   | 按在第 1 行（`top 0`），拖到框下缘外 20px 并**保持不动**          | 内容继续下滚、选区随之增长                      | `top 0 → 15.59 → 50.61 → 76.77 → 111.78 → 120`（= 末行）；`caret 10 → 24 → 35 → 43 → 54`、`selection 7 → 21 → 32 → 40 → 51` ✓ |
| 2   | 到达文本末尾后继续按住                                            | 停在末行、不再滚入空白                          | `top 120`、`caret 54` 六个采样点不变 ✓                                                                                        |
| 3   | 从末尾反向：按住并拖到框上缘外 20px                               | 内容上滚、选区向上长                            | `top 120 → 81 → 46.03 → 10.98 → 0`；`caret 54 → 28 → 14 → 3` ✓                                                                |
| 4   | 单行字段（40 个宽字符，`canvas`）拖到右缘外 40px 并保持           | 内容右滚、光标沿字符推进                        | `scrollLeft 0 → 14.84 → 59.92 → 120.19 → 180.39 → 225.39 → 252.88`（= 上限）；`caret 4 → 24 → 27 → 31 → 35 → 38 → 40` ✓       |
| 5   | 指针**回到框内**后继续按住                                        | 停止滚动（只剩光标跟随指针）                    | `top 107.72` 四个采样点不变、`caret` 随指针落在 33 ✓                                                                          |
| 6   | 只框内拖动（无越界）                                              | 完全不滚动                                      | `top 0`、`scrollLeft 0` 不变 ✓                                                                                                |
| 7   | 触摸拖动到框下缘外并按住（CDP 真触摸）                            | 与鼠标同一条路径                                | `top 0 → 10.32 → 51.58 → 92.8 → 120`；`caret 0 → 24 → 35 → 47 → 54` ✓                                                         |
| 8   | `readOnly` 的 Canvas 字段（有 252.88px 可滚空间）拖到右缘外并按住 | 不认领拖动、不滚动                              | `scrollLeft 0` 不变、`caret 0` ✓                                                                                              |
| 9   | DOM 桥字段（`notes`）拖到框外                                     | 浏览器自己处理，Canvas 字段一个数字都不动       | 三个 Canvas 字段的 `scroll` 全部不变 ✓                                                                                        |
| 10  | `#/options` 正文舞台里的字段拖动 / 旁边的面板拖动                 | A/B：字段滚自己、页面不动；面板滚页面、选区不动 | 字段：`selection 0 → 27`、页面 `stage 900` 不变；面板：页面 `stage 900 → 1075`、`selection` 仍是 27 ✓                         |

**阳性对照**：把逐帧钩子的注册注释掉再跑第 1 条，读数冻结在 `top 0` / `caret 14` / `selection 11`（五个采样点不变）；恢复注册后同一串操作重新变成 `top 0 → 55.01 → 107.45 → 120`。也就是说这条门禁测的确实是新加的那段逐帧推进，而不是别的什么东西恰好把数字改了。

探针：`#demo-state` 的 `canvas.scroll` / `canvasArea.scroll` / `canvasRo.scroll` 逐帧发布 `x,y`（`window.form.canvas().<key>.scrollLeft/scrollTop` 同一来源，来自新公开的 `TextInputBase#scrollLeft/#scrollTop`）；`window.form.rect(key)` 给实时页面矩形（拖动的起止点必须同帧读，§8.47），`window.form.setCanvas(key, value)` 用字段自己的 `setValue` 铺一份可复现的长文本。

### 5.8 走查时踩到的探针坑（不是产品缺陷）
...
**第 97 轮复跑**：`pnpm -r run test` **1303** 通过（core 281 / layout 314 / phaser 330 / widgets 378）、`typecheck` 5/5、`prettier --check .`、`docs:check`（标识符 + idiom + vocabulary 三关）、`pnpm run build:examples`、`pnpm size`（18.6 / 27.1 KB min+gzip）、`node scripts/visual-check.mjs`、22 场景扫场 0 异常 + `lifecycle`/`keyboard`/`uiscene`/`modal`/`pages`/`router`/`list`/`showcase` churn 前后逐项相同 —— 全部通过。
