...
### 5.6 已知局限（未实现，登记为 V58）

纯 Canvas 路径**没有指针选择**：没有拖动选择（`pointerdown` 之后没有 `pointermove` 处理），也没有双击选词。选中只能靠键盘（`Shift+方向键`、`Ctrl+A`）。实测：在文本上从 x = 460 拖到 560，`selection` 始终 0；双击只把光标放到点处（`caret 4`、`selection 0`）。

DOM 桥路径不受影响（浏览器原生选择）。要补这条得先有"拖动归属"的跨控件协议（现在只有 `ScrollView` 内部有：嵌套口可让出拖动），否则在可滚动页面上拖选会**同时**滚动页面——所以本轮只登记，不实现。
