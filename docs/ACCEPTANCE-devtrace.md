...
| `reveal: <port> offset a -> b for <widget>`    | `ScrollView#revealDescendant`（第 67 轮）                      | 焦点被自动滚进视野是"看不见的机制"：它要么让焦点可见，要么因为某种原因不动。这一行给出**是哪个口、从多少到多少、为了哪个控件**，而且只在**真的移动了**时打印（被自己的上下限夹回、其实没动的请求不打——否则会读成 `0 -> 0` 这种"移动了但没有"） |
...
第 67 轮追加（`#/scroll` 上连按 12 次 `Tab`，每次焦点变化都问一遍滚动口）：

```
reveal: scroll.h offset 0 -> 36 for chip.5      ← 焦点走到口右边缘外：横向口跟进一步
reveal: scroll.h offset 36 -> 140 for chip.6
reveal: scroll.h offset 140 -> 244 for chip.7
```

同一段操作在 `setDevMode(false)` 之后 **0 行**（`reveal:` 行数从 7 变 0）。
