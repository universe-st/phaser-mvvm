...
## 10. 第 102 轮：页面栈盖住的页也必须离开无障碍树（V73）

第 101 轮修掉"模态没让被盖住的页面失效"（§9）之后，同一族的第二个实例几乎是立刻能问出来的：**页面栈呢？** 实测（`#/pages`，CDP `Accessibility.getFullAXTree`，只数 `textbox`/`button`）：

| 操作                       | 修前                                 | 修后                                                     |
| -------------------------- | ------------------------------------ | -------------------------------------------------------- |
| 列表页（栈底）             | 17                                   | 17                                                       |
| `pages.open(1)` 推入详情页 | **20** = 列表 17 **+** 详情页 3      | **3**（只有详情页的 `返回列表`/`再进一层`/`打开对话框`） |
| `pages.deeper()` 第三层    | **22** = 列表 17 + 详情 3 + 第三层 2 | **2**                                                    |
| 在详情页上再开一个对话框   | （未修前为 20 + 1）                  | **1**（对话框自己的 `知道了`）                           |
| `pages.popToRoot()`        | 17                                   | 17                                                       |

根因同族：页面栈让被盖住的页继续活着（这正是它保留状态的原因），但"活着"不等于"该被读到"。判据与模态**不同**，这一点是本轮的关键：

- 模态：层以外的**一切**都不可达（指针与焦点陷阱就是这么做的）；
- 页面：只有**栈里 `active === false` 的那些页**不可达 —— 与页面并排的 HUD / 底栏是活界面的一部分，必须留下。所以判据不能写成"顶层页以外一律隐藏"。

修法：`A11yBridge#isInert()`（`packages/phaser/src/a11y.ts`）一个谓词同时回答两问，模态优先；`inert` 照旧进缓存签名，模态与页面栈本来都会 `refreshInteraction()`。**常驻门禁**：`AX_EXPECTATIONS` 新增 `pages`（setup 用 `window.pages.open(1)`，只列详情页的 3 个按钮，AX 门禁的"恰好这么多"语义让"被盖住的列表还在树里"当场变红）。**正对照**：`isInert` 强制 `false` → `modal` 与 `pages` 两条同时红（`2 check(s) failed`），恢复后 `4` 张 AX 表全绿（`modal` 2 / `a11y` 14 / `keyboard` 38 / `pages` 3）。

### 10.1 已知未做：对话框**自身**在树里没有容器角色

现在读屏用户走到对话框里会听到"取消 按钮 / 删除 按钮"，但听不到"这是一个对话框，标题是删除这一项？"——**镜像是一张平铺的 div 列表**（每个控件一个兄弟节点），所以 `role="dialog"` 即使加上去也只是"一个没有子节点的对话框"，`aria-modal` 的包含关系同样表达不出来。要做对得把镜像改成与控件树同构的 DOM 树（节点挂到最近的祖先节点上），那是一次有风险的改动，已登记为下一轮的工作，不在本轮假装做完。→ **第 103 轮已做**，见 §11。

---

## 11. 第 103 轮：镜像长成一棵与控件树同构的树（容器角色、分组、`aria-owns`）

§10.1 登记的增强本轮交付：镜像不再是"同一个隐藏 div 下的兄弟节点清单"，而是**每个节点挂在最近的、有镜像节点的祖先控件之下**（`A11yBridge#nest()`）。同时翻出一个真缺陷 V74（详见 §11.3）。

### 11.1 三层容器角色都到位了

| 容器                     | 修前                                                         | 修后                                                                              |
| ------------------------ | ------------------------------------------------------------ | --------------------------------------------------------------------------------- |
| 模态的 content 根        | 没有容器节点（`confirm` 的两个按钮与页面按钮平铺在一起）     | `dialog:删除这一项？`，`aria-modal=true`，里面正好 2 个 `button`（`取消`/`删除`） |
| `#/a11y` 的 `ScrollView` | `region: 按钮区域` 的 `childIds` 里 0 个控制节点（6 个兄弟） | 6 个 `button` 全在 `region` 里面                                                  |
| `#/a11y` 的字段容器      | 不存在（`Column` 无法命名）                                  | `group:字段区域`，里面 2 个 `textbox`（`aria-owns`，见 §11.2）                    |

对话框的名字就是 content 根的 `label`（`Panel({ label: '删除这一项？' })`）——**没有**给 `ModalOptions` 加新选项：`label` 本来就是基类选项，`ModalHost` 不需要懂 ARIA。`#/modal` 的五种对话框与 `#/keyboard` 的对话框都补了 `label`，无名的对话框仍然合法（实测 Chrome 给出空名字，而不是退回到容器自己的调试名 `ui.page#3`）。

### 11.2 文本框：`aria-owns`（说明为什么不是搬 DOM）

文本框的表面是它自己的 `<input>`（V42），那个元素在 overlay 里、与镜像根**并排**，所以它没法成为对话框节点的 DOM 子节点。ARIA 为此准备了 `aria-owns`：`nest()` 把这类元素挂到"它本该属于的那个镜像节点"上，只改**无障碍树里的父子关系**，DOM 位置、样式与焦点一概不动（把活的 `<input>` 真搬进一个 1px 裁剪的祖先里，会动到浏览器定位、IME 与选区）。

实测（`#/keyboard` 的对话框，CDP `Accessibility.getFullAXTree`）：不用 `aria-owns` 时 35 个控制节点里有 34 个在对话框里，**对话框自己的 `角色名` 是与对话框并排的**；用上之后 **35/35**。同一测量还确认第 101 轮的判据没有回退：页面上的 `玩家名` 依然不在树里。

### 11.3 V74：容器的 `label` 被静默丢弃

`label` 是基类选项，选项审计把"任何选项袋都接受它"写成了规则（`BASE_WIDGET_OPTION_KEYS`），但 `Column`/`Row`/`Grid`/`Stack`/`Absolute` 的选项类型里没有它：`tsc` 报错，绕开类型传进去则被 `splitOptions` 当成布局参数、由 `normalizeParams` 静默忽略。也就是说**审计说这个键没问题，控件却把它扔掉了**——而后果正好落在本轮：容器无法成为可命名的分组。

修法：四个容器补 `ContainerWidgetOptions`（`name` + `label`）并在构造时转交基类；`splitOptions` 把 `label` 与 `name` 一样跳过；`Widget.describeA11y()` 的默认实现补一条规则——**没有自己的描述、但被给了可访问名字的控件镜像为具名 `group`**（`Panel.describeA11y()` 的非交互分支改为回落到 `super`，于是"非交互但有名字的面板"也是分组）。`group` 不是控制角色，所以 `AX_EXPECTATIONS` 的计数（`a11y` 14）没有变化——这正是"新增的东西必须可断言"的用法：计数断言只保证"没有多出控制节点"，包含关系由下面的结构断言负责。

### 11.4 常驻门禁与两条正对照

`scripts/visual-check.mjs` 新增 `AX_STRUCTURE_EXPECTATIONS`（与"控制节点恰好这么多"的 `AX_EXPECTATIONS` 并列）：断言的是**包含关系**——`a11y` 的 `region:按钮区域` 里 6 个 `button`、`group:字段区域` 里 2 个 `textbox`；`modal` 的 `dialog:删除这一项？`（`aria-modal=true`）里 2 个 `button`。实现按 `childIds` 递归统计后代角色（Chrome 会插入自己的 `StaticText`/`InlineTextBox` 包装节点，所以不能只数直接子节点）。

两条正对照各验一半，证明它们测的不是同一件事：

| 正对照                                  | 结果                                                                            |
| --------------------------------------- | ------------------------------------------------------------------------------- |
| `mirrorParentOf` 直接返回根（等于平铺） | **3 条全红**（`3 check(s) failed`），而所有计数断言依然绿 —— 旧门禁测不到这件事 |
| 只关掉 `aria-owns`                      | **只有 `字段区域` 那条红**（`1 check(s) failed`），嵌套两条照旧绿               |

还原后整套 `visual-check` 全绿（`exit 0`）：`modal` 2 / `a11y` 14 / `keyboard` 38 / `pages` 3，四张表 + 结构断言一起通过。

### 11.5 两条实现细节（都会影响别的门禁）

- **`aria-hidden` 会继承**：嵌套之后，"祖先被隐藏"会连带隐藏子孙，所以 §9/§10 的"持有焦点的控件永不隐藏"必须扩展到**祖先链**（`isInert()` 的 `holdsFocus`）。这不是假想情形：焦点还在被盖住那一页的字段上、而模态刚打开的那一帧，外层 `ScrollView`（`role="region"`）正是"祖先"。
- **重排只在结构变化时做**：`nest()` 由 `refresh()`（结构变化）与 `sync()` 中"有节点离开镜像"触发。稳定帧里一个节点都不动——重新 `appendChild` 是一次 DOM 变更，而 DOM 变更正是唤醒无障碍树的东西。另外 `sync()` 的全量遍历改成**倒序**：正向遍历时 `applyDescriptor(null)` 就地 splice，会跳过一个被销毁的控件，让它带着最后一版属性留在树里。
