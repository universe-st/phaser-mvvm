...
## 9. 量程作为数据槽（第 99 轮：修掉 V67/V68/V69）

`Slider.min`/`max` 进 DSL 槽位（`Slider({ min: () => …, max: () => … })`）时，`#/compose` 的 State slots 分区多了一对**同值不同量程**的滑块（`state.rangeA` 量程 100、`state.rangeB` 量程由 `rangeMax` 状态驱动），采样点是**宽度 30% 处**：值 40 时 A 的填充到 40%（采到 `primary`）、B 的填充到 20%（采到轨道色）。一帧里两只并排，所以"忘了重画"会让它们变成同一个颜色 —— 这就是本轮三个缺陷的抓法。

### 9.1 修前（未改动的代码，实测）

| 操作                                  | 读数                                                                    |
| ------------------------------------- | ----------------------------------------------------------------------- |
| 值 40、量程 0..100 → 采样 25% / 45%   | `#2f6feb`（填充）/ `#30363d`（轨道）✓                                   |
| `setRange(0, 200)`（值仍在区间内）    | 采样 25% **仍是 `#2f6feb`** —— 填充仍盖住 40% 的轨道（应 20%）**✗ V67** |
| 同一操作的 `aria-valuemax`            | 仍是 `"100"` **✗ V68**                                                  |
| 值 90、`setRange(0, 30)`（应钳到 30） | 控件 30，绑定 `ref` 仍是 **90**（页面发布 `volume=90`）**✗ V69**        |
| 绑定 `ref` 上 `setValue(80)`          | 控件 80、`ref` 40，且不会回弹 **✗ V69**                                 |

### 9.2 修后（同一页面、同一操作）

| 操作（真鼠标点按钮 / 状态写入）      | 读数                                                                                                                                                |
| ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| 起始：值 40，A 量程 100 / B 量程 100 | A 与 B 在 30% 处**都是** `#2f6feb`（对照：此刻两者应当相同）                                                                                        |
| 点「上限 100」→ B 量程 200           | A 30% 处 `#2f6feb`（填充），B 30% 处 **`#30363d`**（轨道）—— 填充正好减半 ✓；`state.rangeBMax=200`、`state.rangeBValue=40`、`aria-valuemax="200"` ✓ |
| 再点一次 → 回到 100                  | A/B 又相同，`aria-valuemax="100"`（双向都跟随）✓                                                                                                    |
| 值 90 + 量程 0..200（45%）           | 30% 处重新变成填充 ✓                                                                                                                                |
| 值 90 → 量程 30（钳制）              | 控件 **30**、绑定 `ref` **30**、`aria-valuemax="30"`；+500 ms 后仍是 30（不反弹）✓                                                                  |
| 量程给回 100                         | 值保持 30（不会被"发明"出一个新区间内的值）✓                                                                                                        |
| 拖动轨道到 75%                       | 控件 75.47、`ref` 75 ✓（用户路径未受影响）                                                                                                          |
| 聚焦后 `←` 两次（50 起）             | 40 / 40 ✓（键盘路径未受影响，每步 = 量程 1/20）                                                                                                     |

### 9.3 常驻门禁与正对照

`scripts/visual-check.mjs` 的 `#/compose` 采样点新增两条（明暗各一套，`SCENE_SETUP` 里把 `rangeMax` 设成 200）：

| 采样点         | 位置           | 暗        | 亮        | 它保护什么                                                      |
| -------------- | -------------- | --------- | --------- | --------------------------------------------------------------- |
| `state.rangeA` | 宽度 30%, 中线 | `#2f6feb` | `#0969da` | 量程 100、值 40 → 填充应覆盖 30% 处（`primary` 令牌）           |
| `state.rangeB` | 宽度 30%, 中线 | `#30363d` | `#d0d7de` | 量程 200、值 40 → 填充止于 20%，30% 处应是轨道（`border` 令牌） |

两条都在同一张截图里，且期望值互为对照：**只有"量程参与重画"才能同时成立**。正对照用的是本轮开头在**未改动代码**上的实测（§9.1 第 2 行）——把 `setRange` 的重画与缓存键里的量程去掉，B 的采样立刻变回 `#2f6feb`。

> 这三条都**无法在 Node 单测里回归**（`Slider` 需要一个 Phaser 场景），所以回归保护落在浏览器侧：像素门禁（A/B 同帧）+ `#demo-state` 的 `state.rangeBMax`/`state.rangeBValue`/`aria-valuemax` 读数。读数刻意取**控件自己**的 `max`/`getValue()`，而不是驱动它的那个 `ref`。
