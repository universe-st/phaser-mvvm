...
6. **`packages/widgets` 的 `test` 脚本带 `--passWithNoTests`**：这个 flag 只是「暂时没有测试也不至于失败」，四个包现在都有实打实的用例（合计 1289：core 281 / layout 314 / phaser 329 / widgets 365），**不要**为了「有测试」而写空断言。
