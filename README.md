...
>
> **在游戏里使用**：`packages/*` 已配好 `exports`/`publishConfig`（ESM + CJS + `d.ts`），但版本仍是 `0.0.0` 且**未发布到 npm**。在发布之前，请在 monorepo 内以 workspace 协议引用（`"@phaser-mvvm/widgets": "workspace:*"`），或直接使用本仓库的示例工程。`@phaser-mvvm/phaser` 与 `@phaser-mvvm/widgets` 把 `phaser@^4.2.0` 声明为 **peer dependency**，需要由你的工程提供。
