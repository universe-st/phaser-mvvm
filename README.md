...
> 验收场：`#/compose`（DSL，含工厂 API 与 DSL 的逐节点 parity 校验）、`#/showcase`（全部控件与布局形态，已迁移到 DSL）、`#/states`（交互状态矩阵）、`#/lifecycle`（创建→销毁 100 次泄漏门禁），以及 `#/gallery`、`#/dashboard`、`#/bindings`、`#/form`、`#/list`、`#/scroll`、`#/modal`（模态对话框：焦点陷阱 / 遮罩拦截 / `Esc` 关闭 / 叠层 / 100 次开关无泄漏）、`#/pages`（页面栈：列表→详情→对话框、返回时状态与焦点复原、`Esc` 逐层路由）。
